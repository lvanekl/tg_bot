...
def return_as_dictionary(columns_variable: str):
    def decorator(func):
        def function_wrapper(*args, **kwargs) -> list:
            self = args[0]
...
class DB:
    ...
    @connect_to_db
    def new_chat(self, telegram_chat_id: int) -> str:
    ...
    @connect_to_db
    def edit_chat_settings(self, telegram_chat_id: int, **kwargs) -> str:
    ...
    @connect_to_db
    def add_schedule(self, telegram_chat_id: int, weekday: int, sport: str, gym: int, time: Time) -> str:
        assert 1 <= weekday <= 7
    ...
    @connect_to_db
    def remove_schedule(self, schedule_id: int) -> str:
    ...
    def edit_schedule(self, schedule_id: int, new_weekday: int = None,
                      new_sport: str = None, new_gym: int = None, new_time: Time = None) -> str:
    ...
    def add_schedule_correction(self, telegram_chat_id: int, correction_type: str,
                                ...
                                new_date: Date = None, new_time: Time = None, new_gym: int = None) -> str:
    ...
    @connect_to_db
    def remove_schedule_correction(self, schedule_correction_id: int) -> str:
    ...
    def edit_schedule_correction(self, schedule_correction_id: int, new_correction_type: str,
                                 ...
                                 new_new_date: Date, new_new_time: Time, new_new_gym: int) -> str:
    ...
    @connect_to_db
    def remove_admin(self, telegram_chat_id: int, telegram_user_id: int) -> str:
    ...
    @connect_to_db
    @return_as_dictionary(columns_variable="ANSWER_ALTERNATIVE_COLUMNS")
    ...
    @connect_to_db
    def remove_answer_alternative(self, telegram_chat_id: int, answer_alternative_id: int):
        self.cur.execute('''DELETE FROM "answer_alternative" WHERE "id" = ?''', answer_alternative_id)
        return "Удален вариант ответа"

    def get_answer_alternatives_grouped_by_types(self, telegram_chat_id: int):
